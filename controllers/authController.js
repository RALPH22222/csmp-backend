...
import { PHP_TOKEN_ADDRESS, stellarRpc, networkPassphrase } from '../config/stellar.js';
...
async function setupUserStellarAccount(publicKey, secretKey) {
  try {
    // 1. Fund via Friendbot (testnet only)
    ...
    // 2. Create trustline to PHP token
    console.log(`🔗 Adding PHP trustline for ${publicKey}...`);
    const trustOp = invokeOp(PHP_TOKEN_ADDRESS, 'trust', [publicKey]);
    ...
    if (sendResp.status !== 'PENDING') {
      console.error('Trustline send failed');
      return;
    }
...
export const verifyOtp = async (req, res) => {
  try {
    ...
    setupUserStellarAccount(record.userData.stellar_public_key, record.stellarSecretKey)
...
// (unchanged – they remain the same as previous version)
// --------------------------------------------------------------
